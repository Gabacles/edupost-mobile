...
export default function TeachersScreen({ navigation }: Props) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      showModal('Erro', err?.response?.data?.message ?? 'Falha ao criar professor');
    }
  };
  ...
  const handleDelete = (id: string) => {
    setDeleteId(id);
    showModal('Confirmação', 'Deseja excluir este professor?', () => setDeleteId(null));
  };
  const confirmDelete = async () => {
    ...
    try {
      ...
      showModal('Erro', err?.response?.data?.message ?? 'Falha ao excluir professor');
    }
  };
  ...
  const handleEditSave = async (id: string) => {
    try {
      ...
      showModal('Erro', err?.response?.data?.message ?? 'Falha ao atualizar professor');
    }
  };
